...
async function makeAd({ img, name, lang, tag, headline, sub, cta, phone }) {
  const src = `${IN}/${img}`;
  if (!existsSync(src)) { console.log("SKIP (falta fondo)", img); return; }
  const meta = await sharp(src).metadata();
  const W = 1080, H = meta.height >= 1600 ? 1920 : 1080;
...
const ads = [
  // Concepto 1 — robot
  { ...ES, img: "concepto-1.png", name: "cv-1-robot", tag: "FILTROS ATS",
  ...
  { ...EN, img: "concepto-1.png", name: "cv-1-robot", tag: "ATS FILTERS",
  ...
  { ...ES, img: "concepto-2.png", name: "cv-2-antes-despues", tag: "ANTES / DESPUÉS",
  ...
  { ...EN, img: "concepto-2.png", name: "cv-2-before-after", tag: "BEFORE / AFTER",
  ...
  { ...ES, img: "concepto-3.png", name: "cv-3-monton", tag: "100 CANDIDATOS",
    headline: [{ t: "¿Por qué siempre" }, { t: "eligen a otros?", c: "#fbbf24" }],
    sub: "De 100 CVs, la mayoría nunca llega a ojos humanos." },
  { ...EN, img: "concepto-3.png", name: "cv-3-pile", tag: "100 APPLICANTS",
  ...
  { ...ES, img: "concepto-4.png", name: "cv-4-segundos", tag: "6 SEGUNDOS",
    headline: [{ t: "Rechazado" }, { t: "en segundos.", c: "#ef4444" }],
    sub: "El ATS decide antes de que el recruiter abra tu candidatura." },
  { ...EN, img: "concepto-4.png", name: "cv-4-seconds", tag: "6 SECONDS",
