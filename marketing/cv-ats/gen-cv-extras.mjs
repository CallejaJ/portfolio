...
const textBlock = ({ lang, tag, headline, sub, cta, phone }) => {
  ...
  return `
    ${faro(60, 60, 1.4)}
    <text x="135" y="104" font-family="Liberation Sans" font-size="32" font-weight="bold" fill="#e9e4f8">${lang === "es" ? "El faro de tu CV" : "A lighthouse for your CV"}</text>
...
async function makeOfertaStory(lang, outDir) {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    ...
    ${faro(60, 60, 1.4)}
    <text x="135" y="104" font-family="Liberation Sans" font-size="32" font-weight="bold" fill="#e9e4f8">${es ? "El faro de tu CV" : "A lighthouse for your CV"}</text>
    <!-- Documento con check -->
...
for (const lang of ["es", "en"]) {
  ...
  for (const ad of stories[lang]) await makeStory({ ...ad, lang }, dir);
